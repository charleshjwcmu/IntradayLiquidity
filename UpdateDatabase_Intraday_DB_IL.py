...
#excel format table
def update_fx_excel(table_name,output_table_name,date_list=None):
#    date_list=["20170930"]
#    days_need_update = 1500
    if date_list == None:
        base = datetime.datetime.today()
        date_list = [(base - datetime.timedelta(days=x)).strftime("%Y%m%d") for x in range(0, days_need_update)]
    ...
    out_directory_db = PATH_DB+output_table_name+"/HdfData"
    if not os.path.exists(out_directory_db):
        os.makedirs(out_directory_db)

    out_directory_excel = PATH_DB+output_table_name+"/RawData"
    if not os.path.exists(out_directory_excel):
        os.makedirs(out_directory_excel)
    
    for date in date_list:
    #    date = date_list[19]
        file_name_source = raw_directory+"/"+table_name+"_"+date+".xlsx"
        filename_fx = out_directory_excel+"/"+output_table_name+"_"+date+".xlsx"
        file_name_output = out_directory_db+"/"+output_table_name+"_"+date+".hdf"
...
#            print(data_excel["AS_OF_DATE"].unique())
            start_time = time.time()
            data_excel.head()
            fx_rates = data_excel[["CURRENCY_CODE","CURRENCY_SPOT_RATE_USD"]].drop_duplicates().set_index("CURRENCY_CODE").sort_index()
            fx_rates.columns=data_excel["ASOF"].drop_duplicates().values
            fx_rates.to_excel(filename_fx)
            fx_rates.to_hdf(file_name_output,"w",table=True)
            print(table_name+"/"+"FX using " + str(round(time.time()-start_time,0)) + " seconds")
...
def update_fx_tables():
    table_names = ["DM_RPT_IDLM_A1_TRANSACTION_SUMMARY_DETAIL_HIST"]
    output_table_name = "FX_RATES_Intraday"
    for table_name in table_names:
        update_fx_excel(table_name,output_table_name)
...
#    date_list = formatDate(dates[table_name])
    ...
    if table_name in ["DM_RPT_IDLM_A1_TRANSACTION_SUMMARY_DETAIL_HIST","BCBS_248_Data_Extract_A1_Report",\
                      "DM_RTIM_DOM_ALL_TRANSACTION_STEP3","DM_RTIM_DOM_ALL_TRANSACTION_STEP3_MULTI_DAY","FX_RATES_Intraday"]:
        out_directory = PATH_DB+table_name+"/HdfData/"
        ...
        data_all = read_database(file_list)
                        ...
                        'IL_TRANSACTION_AMOUNT_By_TRANSACTION_TYPE',\
                        'IL_TRANSACTION_AMOUNT_By_IBS_NOSTRO_NAME']:
        ...
        data_all = data_all.transpose().sort_index().transpose()
                        ...
                        'IL_TRANSACTION_AMOUNT_By_TRANSACTION_TYPE_and_FMU',\
                        'IL_TRANSACTION_AMOUNT_By_TRANSACTION_TYPE_and_CURRENCY_CODE',\
                        'IL_TRANSACTION_AMOUNT_By_TRANSACTION_TYPE_and_AGENT_BANK_NAME',\
                        'IL_TRANSACTION_AMOUNT_By_TRANSACTION_TYPE_and_IBS_NOSTRO_NAME']:
        out_directory = PATH_DB+"INTRADAY_TABLES/RawData/"
        file = out_directory + table_name + ".xlsx"
        if not os.path.exists(file):
            return()
        ...
        data_all = data_all.transpose().sort_index().transpose()
    elif table_name in ['IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD',\
                        'IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD_CREDIT',\
                        'IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD_DEBIT',\
                        'IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD_byMinute',\
                        'IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD_CREDIT_byMinute',\
                        'IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD_DEBIT_byMinute',\
                        'IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD_with_FedBalance',\
                        'IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD_CREDIT_with_FedBalance',\
                        'IL_RUNNINGTOTAL_DER_TRANSACTION_AMOUNT_USD_DEBIT_with_FedBalance',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT_CREDIT',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT_DEBIT',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT_byMinute',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT_CREDIT_byMinute',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT_DEBIT_byMinute',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT_with_FedBalance',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT_CREDIT_with_FedBalance',\
                        'IL_RUNNINGTOTAL_TRANSACTION_AMOUNT_DEBIT_with_FedBalance']:
...
#        data_all.index = [datetime.datetime.strptime(x, "%H:%M:%S").time() for x in data_all.index]
        ...
        data_all = pd.ExcelFile(file).parse("Sheet1",index_col=0)
        data_all = data_all.transpose().sort_index().transpose()
    elif table_name.startswith('IL_RUNNINGTOTAL_'+column_name_transaction_amount) or table_name.startswith('IL_RUNNINGTOTAL_'+column_name_transaction_amount_local):
        out_directory = PATH_DB+"INTRADAY_TABLES/RawData/"
        file = out_directory + table_name + ".xlsx"
        if not os.path.exists(file):
            return()
        ...
        data_all = data_all.transpose().sort_index().transpose()
                        ...
                        'IL_TRANSACTION_AMOUNT_By_FMU_and_TRANSACTION_TYPE_and_ACCOUNT_TYPE_and_CURRENCY_CODE',\
                        'IL_TRANSACTION_AMOUNT_By_AGENT_BANK_NAME_and_TRANSACTION_TYPE_and_ACCOUNT_TYPE_and_CURRENCY_CODE',\
                        'IL_TRANSACTION_AMOUNT_By_FMU_and_TRANSACTION_TYPE_and_CURRENCY_CODE']:
        out_directory = PATH_DB+"INTRADAY_TABLES/RawData/"
        file = out_directory + table_name + ".xlsx"
        if not os.path.exists(file):
            return()
...
#        data_all = data_all.transpose().sort_index().transpose()
    elif table_name in ["SoD_Balance_International"]:
        file_name = "//mfmaog02/ERMShared/Treasury Risk/Charles/IntradayLiquidity/Data/Opening Balance_by_Reporting Group.xls"
        data_all = pd.ExcelFile(file_name).parse("Opening Balance_by_Reporting Gr",index_col=0)
    elif table_name in ["SoD_Balance_International_Local_Currency"]:
        file_name = "//mfmaog02/ERMShared/Treasury Risk/Charles/IntradayLiquidity/Data/Opening Balance_by_Reporting Group_Local_Currency.xls"
        data_all = pd.ExcelFile(file_name).parse("Opening Balance_by_Reportin_loc",index_col=0)
    else:
