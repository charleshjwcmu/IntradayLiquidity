...
#        start_date = datetime.datetime.strptime('2016-01-04 00:00:00', '%Y-%m-%d %H:%M:%S')
        start_date = datetime.datetime.strptime('2018-07-01 00:00:00', '%Y-%m-%d %H:%M:%S')
    else:
        start_date = datetime.datetime.strptime('2016-01-04 00:00:00', '%Y-%m-%d %H:%M:%S')
...
#    
    ...
    balance_type = column_name_transaction_amount
    attribute_groupby = [column_name_transaction_type,column_name_currency]
    file_total_deposit_client = misc_dir+"/RawData/IL_"+balance_type+"_By_"+ "_and_".join(attribute_groupby)+".xlsx"
    update_intraday_stat_from_IL_A1_Extract_Table(file_total_deposit_client,balance_type,attribute_groupby)
    # type * Data Mart
    balance_type = column_name_transaction_amount
...
# LNNCP
def update_intraday_stat_from_IL_A1_Extract_Table3(file_total_deposit_client,transaction_type=None,table_name = "DM_RPT_IDLM_A1_TRANSACTION_SUMMARY_DETAIL_HIST"):
#   attribute_groupby = [column_name_legal_entity]
    if PRODUCTION_ENVRIONMENT:
...
#                    transaction_type = "CREDIT"
                ...
                running_total_day['STT-Total'] = table_tmp.groupby(column_name_transaction_time).agg({column_name_transaction_amount:sum})[column_name_transaction_amount].cumsum()

#                running_total_day.plot()

                fmus = table_tmp[[column_name_fmu]].drop_duplicates()[column_name_fmu]
                for fmu in fmus:
                    running_total_day[fmu] = table_tmp[table_tmp[column_name_fmu]==fmu].groupby(column_name_transaction_time).agg({column_name_transaction_amount:sum})[column_name_transaction_amount].cumsum()
#                legal_entity = table_tmp[[column_name_legal_entity]].drop_duplicates()[column_name_legal_entity]
#                for le in legal_entity:
#                    running_total_day[le] = table_tmp[table_tmp[column_name_legal_entity]==le].groupby(column_name_transaction_time).agg({column_name_transaction_amount:sum})[column_name_transaction_amount].cumsum()
#
#                currency = table_tmp[[column_name_currency]].drop_duplicates()[column_name_currency]
#                for cur in currency:
#                    running_total_day[cur] = table_tmp[table_tmp[column_name_currency]==cur].groupby(column_name_transaction_time).agg({column_name_transaction_amount:sum})[column_name_transaction_amount].cumsum()
                
                running_total_day = running_total_day.fillna(method='ffill').fillna(0)
                
#                running_total_day['EUR-Deutsche'].plot()
...
#                    legal_entity = table_tmp[[column_name_legal_entity]].drop_duplicates().tolist()
#                    for le in legal_entity:
#                        running_total_day[le] = table_tmp[table_tmp[column_name_legal_entity]==le].groupby(column_name_transaction_time).agg({column_name_transaction_amount:sum})[column_name_transaction_amount].cumsum()
#    
#                    currency = table_tmp[[column_name_currency]].drop_duplicates().tolist()
#                    for cur in currency:
#                        running_total_day[cur] = table_tmp[table_tmp[column_name_currency]==cur].groupby(column_name_transaction_time).agg({column_name_transaction_amount:sum})[column_name_transaction_amount].cumsum()
