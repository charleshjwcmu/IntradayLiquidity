...
#def update_table_SoD_balance():
#    file_name = "SoD_Balance_International"
#    sod_balance = read_db(file_name)
##    sod_balance.index = [x.strftime("%Y%m%d") for x in sod_balance.index]
##    sod_balance.transpose()
#    return(sod_balance)
 
# LNNCP
def update_intraday_stat_from_IL_A1_Extract_Table3(file_total_deposit_client,time_interval = column_name_transaction_time,transaction_type=None,table_name = "DM_RPT_IDLM_A1_TRANSACTION_SUMMARY_DETAIL_HIST",SoD_Balance=False):
...
#                    transaction_type = "CREDIT"
                ...
                if not SoD_Balance:
                    ...
                    running_total_day['STT-Total'] = table_tmp.groupby(time_interval).agg({column_name_transaction_amount:sum})[column_name_transaction_amount].cumsum() + balance
                    fmus = table_tmp[[column_name_fmu]].drop_duplicates()[column_name_fmu]
                    for fmu in fmus:
#                        fmu = fmus[2]
                        if fmu in fed_balance.columns:
                            balance = fed_balance[fmu].values[0]
                            running_total_day[fmu] = table_tmp[table_tmp[column_name_fmu]==fmu].groupby(time_interval).agg({column_name_transaction_amount:sum})[column_name_transaction_amount].cumsum() + balance
...
def update_table3():
    ...
    update_intraday_stat_from_IL_A1_Extract_Table3(file_total_deposit_client,SoD_Balance=True)

    file_total_deposit_client = misc_dir+"/HdfData/IL_RUNNINGTOTAL_"+balance_type+"_CREDIT_with_FedBalance.hdf"
    update_intraday_stat_from_IL_A1_Extract_Table3(file_total_deposit_client,transaction_type="CREDIT",SoD_Balance=True)

    file_total_deposit_client = misc_dir+"/HdfData/IL_RUNNINGTOTAL_"+balance_type+"_DEBIT_with_FedBalance.hdf"
    update_intraday_stat_from_IL_A1_Extract_Table3(file_total_deposit_client,transaction_type="DEBIT",SoD_Balance=True)
