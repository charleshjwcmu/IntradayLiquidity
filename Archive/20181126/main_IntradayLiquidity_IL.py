...
#            lpncp_time = net_cumulative[net_cumulative == lpncp].index[0]
    ...
    #    fed_balance = Fed_balance.loc[date,'Fed_Balance']
        ...
        if fed_balance is not None:
            table_name = "IL_RUNNINGTOTAL_"+column_name_transaction_amount+"_with_FedBalance_"+key
            percentiles_runningtotal = read_db(table_name)
    ...
    #    date = timewindow[0]
        ...
        if date in Fed_balance.index:
            analyze_intraday_data(data_tmp.copy(),file_suffix="STT-Total",key="STT-Total",limit_red_nofed=None, fed_balance=Fed_balance.loc[date,'STT-Total'])
        else:
...
#        fmus = data_tmp["FMU"].drop_duplicates().tolist()
        ...
        for fmu in fmus:
    #        fmu = fmus[1]
            if fmu in Fed_balance.columns and date in Fed_balance.index:
                stat = analyze_intraday_data(data_tmp[data_tmp["FMU"]==fmu],file_suffix=fmu.replace(" ", ""),key=fmu,fed_balance=Fed_balance.loc[date,fmu])
            else:
                stat = analyze_intraday_data(data_tmp[data_tmp["FMU"]==fmu],file_suffix=fmu.replace(" ", ""),key=fmu)
            stat.columns = [fmu]
            if len(stats) == 0:
                stats = stat
            else:
