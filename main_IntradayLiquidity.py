...
#            fig.savefig(plot_folder+"/History_Chart_By_TotalPayments_"+attribute+"_"+sub_category.replace(" ","")+".png",bbox_inches='tight')
    ...
    summary_data = summary_data.transpose()
    summary_data.index = [datetime.datetime.strptime(x,"%Y%m%d") for x in summary_data.index]
    sub_categorys = summary_data.columns.get_level_values(column_name_legal_entity).drop_duplicates().tolist()
...
#                running_tmp = Running[sub_category].head(1000)
        if all(np.isnan(running_tmp)):
            return(np.nan)
...
#        plt.legend(loc="best",prop={'size':10})
    ...
    Fed_balance.index = [datetime.date(x.year,x.month,x.day) for x in Fed_balance.index]

    sub_categories = Running.columns
    for sub_category in sub_categories:
    #    sub_category = 'SSBT BOSTON'
        result_tmp = Running[sub_category].resample('1D').apply([extract_lnncp,extract_lnncp_time,extract_lpncp,extract_lpncp_time,extract_net_cumulative_obligation])
        ...
        if sub_category in ["STT-Total","SSBT BOSTON"]:
            # Calculate minimum balance and plot the chart
            result_tmp = result_tmp.join(Fed_balance,how="left")
            result_tmp = result_tmp[~pd.isnull(result_tmp)]
            result_tmp["MinimumBalance"] = result_tmp["LNNCP"] + result_tmp['Fed_Balance']
            result_tmp.rename(columns={"Fed_Balance":"StartBalance"},inplace=True)
...
#            ax.xaxis.set_major_formatter(myFmt)
        if tmp.index.__class__ is pd.core.indexes.base.Index:
            custom_tick_locs = [datetime.time(hour=x) for x in [0,8,9,12,14,15,16,17,18,19]]
            plt.xticks(custom_tick_locs,rotation=90)
    ...
    def analyze_intraday_data(tmp_data,file_suffix,key,limit_red_nofed = None, fed_balance = None,Verbose=False):
    ...
    #    fed_balance = Fed_balance.loc[date,'Fed_Balance']
        ...
        plt.clf()
        if Verbose:
            data_tmp_nobalance[column_name_running_total].to_excel(plot_folder+"/IntradayLiquidity_running_total_"+date+"_"+file_suffix+".xlsx")
        ...
        if fed_balance is not None:
            ...
            plt.clf()
            if Verbose:
                data_tmp_nobalance[column_name_running_total].add(fed_balance).to_excel(plot_folder+"/IntradayLiquidity_running_total_with_fed_balance_"+date+"_"+file_suffix+".xlsx")

        
        if data_tmp_nobalance.index.__class__ is not pd.core.indexes.base.Index:
            return()
...
#            le = legal_entity[0]
            if le == "SSBT BOSTON":
                ...
                stat = analyze_intraday_data(data_tmp[data_tmp[column_name_legal_entity]==le],le.replace(" ", ""),key=le,fed_balance=0)
